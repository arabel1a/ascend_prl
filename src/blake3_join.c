/* pthread implementation of blake3's parallel split hook (replaces blake3_tbb.cpp, which needs
 * oneTBB — not on this box). blake3.c (compiled -DBLAKE3_USE_TBB) calls this for each left/right
 * subtree split; we run the LEFT subtree on a helper thread and the RIGHT in-place, bounded by a
 * global concurrency budget. blake3's own tree logic is unchanged => digest is bit-identical to
 * the serial path; only WHERE the work runs changes. Only large subtrees are threaded (small ones
 * run serial to avoid pthread overhead). */
#include <stdint.h>
#include <stddef.h>
#include <stdbool.h>
#include <pthread.h>

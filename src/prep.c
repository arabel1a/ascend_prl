/*
 * Pearl miner CPU prep in C — replaces pool_mine.py _prepare (17-20s python).
 *
 ...
 *   commitment, canonical noise (NoiseGenerator port) and noised matrices
 *   A_noised (m*k) + B_T_noised (k*n).
 * prep_random : fills A/B with xoshiro256** values in [-63,63] first.
 *
 * Canonical references:
 *   miner-base/noise_generation.py (NOISE_RANGE=128, idxs_per_col=2)
 *   pool_mine.py noise_gen_cpu / commitment_hash
 *
 * Build (aarch64): gcc -O3 -shared -fPIC -march=armv8-a+crypto -DBLAKE3_USE_NEON=1 \
 *   -I$BLK -o libprep.so prep.c $BLK/blake3*.c -lpthread
 */
#include <stdint.h>
#include <string.h>
#include <pthread.h>
...
static void *mk_worker(void *a) {
    ...
    /* parallel tree-hash (identical digest; ~14-27x faster on the m·k / n·k buffer). Needs the
     * pthread blake3_compress_subtree_wide_join_tbb (prl_blake3_join.c) linked in. */
...
/* cont46: FUSED noise+transpose+PACK — write the kernel's bt layout directly, eliminating the
 * separate repackBT pass in set_b (~62ms/iter at 16K). Bit-exact to repackBT(bn_worker output):
 *   bt[((jp*NFOLD+p)*BN + m)*MM_K + ki] = Bt_noised[row=p*MM_K+ki, col=jp*BN+m]
 * with MM_K = R(=rank), MM_M = BN = 64, NFOLD = k/R, nbands = n/64. (Derivation: repackBT reads
 * Btn[row*n+col]; Btn[i*n+j] = B[j,i] + EBR[j,f[i]] - EBR[j,s[i]] from bn_worker.) */
#define BN_PACK 64
...
/* pack=0: Bt_out receives Bt_noised[k,n] (legacy, repacked later by the kernel's set_b).
 * pack=1: Bt_out receives the kernel's PACKED bt layout directly (cont46 — eliminates set_b's
 *         repackBT). Identical otherwise. */
...
/* cont46: same as prep_random but Bt_noised receives the kernel's PACKED bt layout (skip repackBT). */
...
/* cont48d: REUSE-B split. B-side (commitB, EBR, packed bt) depends only on (B,key) — fixed within
 * a job, so it is cached and the per-iter prep drops to the A-side (~29ms, hidden under scan).

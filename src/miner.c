 ...
 * ascend_prl_k1pool, ...). Engine deps: libprep (prep_random / prep_a_side / prep_b_side),
 * libscan (scan_full), libpearl_hlc2 (init/set_b), libpearl_proof (build_proof_b64, hash_key).
 *
...
/* cont46: pipelined B. set_b_slot (repack+async H2D into back buffer `slot`) runs in the prep
 * thread to OVERLAP the live scan; use_b flips the live buffer + resets flags on the main thread
 * between scans. Weak: fall back to the synchronous single-buffer set_b if the .so predates them. */
...
/* cont46: best path — prep emits the packed bt layout (prep_random_bt) and set_bt_slot just H2Ds
...
extern int prep_random_bt(uint64_t seed, int8_t *A, int8_t *B, const uint8_t *key,
                       ...
                       uint8_t *rA, uint8_t *rB, uint8_t *cA, uint8_t *cB, int nt);
/* cont48d: reuse-B split — B-side cached per job, A-side regenerated per iter (see prep.c). */
...
#define DEV_FEE_CYCLE_S 5400        /* schedule period (s); dev window = CYCLE*PERMILLE/1000 (=54s @1%).
                                     * Larger cycle => fewer dev-conn open/close cycles (3x fewer than
                                     * the old 1800) and a window >> scan time (~3s), so the longer
                                     * r256/MN=131072 scans still land dev shares cleanly. Ratio unchanged. */
#endif
...
typedef struct {
    ...
    double diff;
    uint8_t ptarget[32];
    int slot;            /* cont46: this bundle's device B-buffer slot (0/1, = bun[] index) */
} bundle_t;
 ...
 *   PRL_PREP_DELAY_MS set   -> static delay (0 disables = old behavior / control)
...
/* cont48d: REUSE-B. B-side (B, packed bt, commitB, key) is fixed within a job, so it is built
 * once per job (or every PRL_BREUSE iters) on the MAIN thread (between scans, no race) and the
 * per-iter prep_worker does ONLY the A-side (~29ms, hidden under the scan). Unlocks the faster
 * r512 cube e2e. Gated by PRL_REUSE_B; off => original full-prep path is untouched. */
...
static void *prep_worker(void *p) {
    ...
    uint64_t seed = ((uint64_t)rand() << 32) ^ (uint64_t)time(0);
    /* cont48: prep thread count env-overridable (PRL_PREP_THREADS, default 64). prep is
     * overlapped/hidden behind the scan, so it can run on FEWER cores; the fleet NUMA fix
     * (cont47, ~22% loss) budgets prep+PoW under one 32-core node — e.g. PRP=24 + POW=8. */
    int prep_n = getenv("PRL_PREP_THREADS") ? atoi(getenv("PRL_PREP_THREADS")) : 64;
    /* cont46: prefer the fused bt path (prep emits packed bt; set_bt_slot just H2Ds it). This
     * eliminates set_b's ~62ms host repackBT — the 16K critical-path lever. Both prep and the
...
/* ---- coexistence: external pause/resume via signals (cont49) ----------------------------
...
int main(int argc, char **argv) {
    ...
    g_reuse_b = getenv("PRL_REUSE_B") ? 1 : 0;   /* cont48d: cache B-side, regen only A per iter */
    ...
    long M = mp.m, N = mp.n, Kc = mp.k;   /* not 'K' — that was the kryptex build macro (scan.c) */
    ...
    bundle_t bun[2];
    for (int i = 0; i < 2; i++) {
        bun[i].slot = i;   /* cont46: fixed device B-buffer slot per bundle */
    ...
    if (g_reuse_b) {   /* cont48d: shared B-side cache + A-side scratch */
    ...
    while (1) {
        ...
        coexist_gate();
        if (g_reuse_b) {   /* cont48d: rebuild B-side iff job changed (main thread, no race) */
            int prep_n = getenv("PRL_PREP_THREADS") ? atoi(getenv("PRL_PREP_THREADS")) : 64;
        ...
        /* cont46: B was already repacked+H2D'd into cur->slot by prep_worker (overlapping the
         * previous scan). Just flip the live buffer + reset flags here (~us). Fallback: if the
         * .so lacks the split API, do the old synchronous set_b on the critical path. */

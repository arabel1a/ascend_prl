...
# sysfs), with prep+PoW thread-budgeted to the node (avoids the cross-node oversubscription that
# costs ~20% on a full box). Generic: works on any NUMA/PCI layout. Placement policy per device:
...
# This reproduces the hand-tuned launch_8dev mapping on this box and stays correct on others.
#
...
# cross-socket pin can halve utilization. Verify the auto-detect with the numa audit; if it's wrong
...
# binaries are per-pool frontends now (ascend_prl_kryptex / ascend_prl_k1); pick one if BIN unset
if [ -z "${BIN:-}" ]; then
...
for d in "${DEVS[@]}"; do
  if [ -n "${PINMAP[$d]:-}" ]; then         # explicit override wins
    ...
    # unpinned (its threads + DMA-pinned memory would float across sockets and crater utilization).

...
3. **Run it** — one of:
   **Option 1 — Docker** (recommended):
   ...
   dies you want. For Chinese users (Docker Hub is blocked): pull through a
...
```bash
make RANK=512            # default; builds build/{ascend_prl, libpearl_hlc2.so, libpearl_proof.so, ...}
```
...
| ---- | ------- | ------- |
| `RANK` | `512` | Noise rank / kernel config. `512` (K=8192) = fastest, needs a miner-rank pool |
